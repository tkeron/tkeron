...
export function* getFilesRecursive(
  ...
  while (pending.length) {
    ...
    if (file.isDirectory()) {
      ...
      if (returnOnlyDirs && useDirectoryBase)
        yield pathToUrlResource(path, topDirectory).url;
    ...
    if (useDirectoryBase && !pattern)
      yield returnNamesOnly
        ? file.name
        : pathToUrlResource(join(directory, file.name), topDirectory).url;
    if (useDirectoryBase && pattern && file.name.match(pattern))
      yield returnNamesOnly
        ? file.name
        : pathToUrlResource(join(directory, file.name), topDirectory).url;
    if (!useDirectoryBase && !pattern)
      yield returnNamesOnly ? file.name : join(directory, file.name);
    if (!useDirectoryBase && pattern && file.name.match(pattern))
      yield returnNamesOnly ? file.name : join(directory, file.name);
  }
}
