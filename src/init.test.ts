...
describe("init", () => {
  ...
  it("should copy api-service.ts for pre-rendering with external APIs", async () => {
    ...
    try {
      ...
      expect(existsSync(join(projectPath, "websrc", "api-service.ts"))).toBe(true);
      
      const apiServiceContent = readFileSync(join(projectPath, "websrc", "api-service.ts"), "utf-8");
      expect(apiServiceContent).toContain("getRandomQuote");
      expect(apiServiceContent).toContain("getBuildMetadata");
      expect(apiServiceContent).toContain("https://dummyjson.com/quotes/random");
  
      // index.pre.ts should import from api-service
      const indexPreTs = readFileSync(join(projectPath, "websrc", "index.pre.ts"), "utf-8");
