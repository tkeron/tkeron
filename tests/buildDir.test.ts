...
import { buildDir } from "../src/buildDir";
import { buildEntrypoints } from "../src/buildEntrypoints";
...
describe("buildDir", () => {
  ...
  it("should filter out .com.html files", async () => {
    ...
    try {
      ...
      writeFileSync(
        join(TEST_SRC, "component.com.html"),
        "<div>Component</div>",
      );
...
describe("buildEntrypoints", () => {
  it("should return early and log error when filePaths is empty", async () => {
    ...
    expect(errors.some((e) => e.includes("No entrypoints provided"))).toBe(
      true,
    );
  });
  ...
  it("should return early and log error when filePaths is not an array", async () => {
    ...
    const result = await buildEntrypoints("not-an-array" as any, "/tmp/root", {
      logger,
    });
    expect(result).toBeUndefined();
    expect(errors.some((e) => e.includes("No entrypoints provided"))).toBe(
  ...
  it("should return early and log error when root is not a string", async () => {
    ...
    const result = await buildEntrypoints(["file.html"], null as any, {
      logger,
    });
    expect(result).toBeUndefined();
    expect(errors.some((e) => e.includes("Invalid root provided"))).toBe(true);
  ...
  it("should build entrypoints and return artifacts", async () => {
    ...
    try {
      ...
      writeFileSync(
        join(dir, "index.html"),
        "<!DOCTYPE html><html><body><h1>Test</h1><script src='./main.ts'></script></body></html>",
      );
      writeFileSync(join(dir, "main.ts"), "console.log('hello');");

      const result = await buildEntrypoints([join(dir, "index.html")], dir, {
        logger: silentLogger,
      });
